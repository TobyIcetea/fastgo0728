...
func FuzzResourceID_New(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, counter uint64) {
		...
		// 断言结果必须包含资源标识符前缀
		assert.Contains(t, result, rid.UserID.String()+"-", "The Generated unique identifier should contain the corrent prefix")
		...
		if len(splitParts) == 2 {
			...
			t.Errorf("The format if the generated unique identifier does not meet expectation")
		}
	})
}
